...
ORIENTATION_DATA = [-1.0564747468923541e-06, 8.746699709178704e-07, 0.9999999999990595,  1.5880805820884731]
...
# Channel creation. 
serialMuxProtChannelIdSPEED_SET = smp_server.create_channel(SPEED_SETPOINT_CHANNEL_NAME, SPEED_SET_DLC)
...
def callback_Status(payload: bytearray) -> int:
    ...
    if payload[0] == 1:
        ...
        smp_server.send_data(SPEED_SETPOINT_CHANNEL_NAME, struct.pack('2H', 0, 0))  # Stop motors
        smp_server.send_data(COMMAND_CHANNEL_NAME,struct.pack('B', CMD_ID_SET_READY_STATE))   # SENDING A COMMAND_ID SET READY STATUS
        reinitialize()       
...
def callback_LineSensors(payload: bytearray)-> None:
    ...
    """
    global no_line_detection_count, reset_count
    ...
    if all(value == 0 for value in sensor_data):
        no_line_detection_count += 1
    else:
        no_line_detection_count = 0

    if no_line_detection_count >= 20 or all(value >= LINE_SENSOR_ON_TRACK_MIN_VALUE for value in sensor_data):
        ...
        smp_server.send_data(COMMAND_CHANNEL_NAME,struct.pack('B', CMD_ID_SET_READY_STATE))             
        reset_count += 1
...
def main_loop():
    ...
    """
    status = 0
    elapsedTimeSinceReset = 0  # Elapsed time since reset [ms]

    # Channel subscription.
    smp_server.subscribe_to_channel(STATUS_CHANNEL_NAME, callback_Status)
    smp_server.subscribe_to_channel(LINE_SENSOR_CHANNEL_NAME, callback_LineSensors)
    ...
    if serialMuxProtChannelIdSPEED_SET == 0  or serialMuxProtChannelIdCMD == 0 :
        ...
        if robot_node is None:
            ...
            while supervisor.step(timestep) != -1:
                elapsedTimeSinceReset += timestep
                smp_server.process(elapsedTimeSinceReset)
